...
sequenceDiagram
    ...
    alt INITIALIZATION PHASE (input event exists)
        ...
        rect rgb(200, 220, 240)
            ...
            Agent->>Context: contextBuilder({ lifecycle: 'init', input, tools, span, selfContract })
            Note over Context: Developer-defined function:<br/>Maps input event → System Prompt + Messages + Enabled Tools
            Context-->>Agent: { system: string, messages: AgentMessage[], enabledTools: Record<string, boolean> }
            ...
            Agent->>Agent: applyToolEnablement(tools, enabledTools)
            Note over Agent: Filter tools based on enabledTools map.<br/>Tools not in map or set to true are included.<br/>Tools explicitly set to false are excluded.
        end
        ...
        rect rgb(255, 245, 230)
            ...
            alt explicitPermissionRequired handler defined
                Agent->>Agent: Evaluate explicitPermissionRequired({ services, mcp, tools })
        ...
        rect rgb(240, 220, 200)
            ...
            Note over Agent: Messages restored with their<br/>preserved seenCount values.<br/>enabledTools restored from state.
            ...
            rect rgb(255, 245, 230)
                ...
                alt Service is Permission Manager response
                    ...
                    alt Permission Manager returned system error
        ...
        rect rgb(255, 245, 230)
            ...
            Agent->>Agent: applyToolEnablement(tools, context.enabledTools)
        end
    ...
    rect rgb(220, 240, 220)
        ...
        loop While agentCycles.current < agentCycles.max
            CoreLoop->>CoreLoop: Determine agentCycleQuotaExhausted
            Note over CoreLoop: agentCycleQuotaExhausted = !(current < max)

            rect rgb(230, 245, 255)
                Note over CoreLoop: **Pre-Inference Hook Phase**

                alt preInferenceHook defined
                    CoreLoop->>CoreLoop: preInferenceHook({ messages, system, tools, span, agentCycles, tokenUsage })
                    Note over CoreLoop: Hook can modify messages before LLM call.<br/>Returns modified messages or undefined.<br/>Errors are logged but don't stop execution.
                    CoreLoop->>CoreLoop: messages = hookResult ?? messages
                end
            end

            CoreLoop->>CoreLoop: Stream lifecycle event
            Note over CoreLoop: onStream({<br/>  type: lifecycle === 'init' ? 'agent.init'<br/>       : lifecycle === 'tool_result' ? 'agent.resume'<br/>       : 'agent.self.correction',<br/>  data: { system, messages, tools, agentCycles }<br/>})

            CoreLoop->>LLM: llm({ lifecycle, system, messages, tools, outputFormat, agentCycles, onStream })
            ...
            LLM-->>CoreLoop: { type: 'tool_call' | 'text' | 'json', executionUnits, usage: { tokens: { prompt, completion } } }

            CoreLoop->>CoreLoop: executionUnits += response.executionUnits
            CoreLoop->>CoreLoop: tokenUsage.prompt += response.usage.tokens.prompt
            CoreLoop->>CoreLoop: tokenUsage.completion += response.usage.tokens.completion
            CoreLoop->>CoreLoop: agentCycles.current++

            rect rgb(230, 245, 255)
                Note over CoreLoop: **Post-Inference Hook Phase**

                alt postInferenceHook defined
                    CoreLoop->>CoreLoop: postInferenceHook({ inference, span, agentCycles, tokenUsage })
                    Note over CoreLoop: Hook can inspect inference result.<br/>Returns { action: 'RETRY' | 'CIRCUIT_BREAK', error? }

                    alt Hook returns RETRY
                        CoreLoop->>CoreLoop: Log "RETRY action issued"
                        CoreLoop->>CoreLoop: continue (skip this inference, redo)
                    else Hook returns CIRCUIT_BREAK
                        CoreLoop->>CoreLoop: Log "CIRCUIT_BREAK action issued"
                        CoreLoop-->>Agent: throw pihResult.error
                    end
                end
            end
            ...
            alt Response Type: TOOL_CALL
                ...
                rect rgb(240, 240, 200)
                    ...
                    loop For each prioritized tool call
                        CoreLoop->>CoreLoop: Stream tool request event
                        Note over CoreLoop: onStream({<br/>  type: 'agent.tool.request',<br/>  data: { tool: { name, kind, originalName }, usage, executionunits }<br/>})
                        ...
                        alt Tool doesn't exist
                            ...
                            rect rgb(255, 230, 200)
                                ...
                                Note over CoreLoop: toolsPendingPermission[name] = { definition, requests }
                ...
                alt Arvo service calls exist (including permission requests)
                    CoreLoop->>CoreLoop: Stream delegation event
                    Note over CoreLoop: onStream({<br/>  type: 'agent.tool.request.delegation',<br/>  data: { tools: arvoToolCalls.map(name), executionunits, usage }<br/>})

                    CoreLoop->>Otel: End span
                    CoreLoop-->>Agent: { messages, toolCalls: arvoToolCalls, agentCycles, executionUnits, tokenUsage }

                    rect rgb(200, 240, 240)
                        Note over Agent,Memory: Suspension & Persistence Phase

                        Agent->>Agent: Build AgentState for persistence
                        Note over Agent: {<br/>  initEventAccessControl,<br/>  currentSubject,<br/>  enabledTools,<br/>  system,<br/>  messages, // WITH seenCount preserved per message<br/>  agentCycles,<br/>  awaitingToolCalls,<br/>  totalExecutionUnits,<br/>  totalTokenUsage: { prompt, completion }<br/>}
                ...
                rect rgb(240, 200, 240)
                    ...
                    CoreLoop->>CoreLoop: Stream output finalization event
                    Note over CoreLoop: onStream({<br/>  type: 'agent.output.finalization',<br/>  data: { content: response.content, usage, executionunits }<br/>})
                    ...
                    alt Output validation fails
                        ...
                        CoreLoop->>CoreLoop: Stream output event
                        Note over CoreLoop: onStream({<br/>  type: 'agent.output',<br/>  data: { content: JSON.stringify(data), usage, executionunits }<br/>})

                        CoreLoop->>Otel: End span
                        CoreLoop-->>Agent: { messages, output: validatedData, agentCycles, executionUnits, tokenUsage }
        ...
        alt Agent cycle quota exhausted
